...
local item_width = 44  -- Width per app icon

local get_width = function()
    local app_count = helpers.tablelength(favourites) + helpers.tablelength(source())
    local width = app_count * item_width + 16  -- 16 for padding
...
local redraw_borders = function()
    ...
    if previous_width ~= current_width then
        if helpers.tablelength(source()) == 0 then
            applist.spacing = dpi(0)
        else 
            applist.spacing = dpi(4)
        end

        container.redraw(current_width)
        container.forced_width = current_width + 16  -- Update forced_width for dock
    end
end
...
container.forced_width = get_width() + 16
