...
function _M.preview()
	...
	local x = workarea.x + (workarea.width - W) / 2
	local y = workarea.y + (workarea.height - H) / 2
...
function _M.showPreview()
	...
	-- Timer to hide the preview box after a certain delay
	local timeout_duration = 10 -- Adjust this value as needed (in seconds)
	local hide_timer = timer.start_new(timeout_duration, function()
