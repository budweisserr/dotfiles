...
awful.keyboard.append_global_keybindings {
    ...
    awful.key {
        modifiers = { mod.super },
        key = 'w',
        description = 'move window to second screen',
        group = 'awesome',
        on_press = function()
            local focused_client = client.focus
            if focused_client then
                awful.client.movetoscreen(focused_client, 2)
            end
        end,
    },
    awful.key {
        modifiers = { mod.super },
        key = 'q',
        description = 'move window to primary screen',
        group = 'awesome',
        on_press = function()
            local focused_client = client.focus
            if focused_client then
                awful.client.movetoscreen(focused_client, 1)
            end
        end,
    },
    awful.key {
    ...
    awful.key {
        ...
        group = 'awesome',
        on_press = function()
            awful.spawn.with_shell("loginctl lock-session \"${XDG_SESSION_ID:-self}\" || env XSECURELOCK_SAVER=saver_xscreensaver xsecurelock")
        end,
    },
    awful.key {
    ...
    awful.key {
        modifiers = { "Shift", "Mod1" }, -- Mod1 is usually the Alt key
        key = 'Mode_switch',             -- This may vary depending on your keyboard layout
        description = 'change keyboard layout',
        group = 'launcher',
        on_press = function()
            awful.spawn("setxkbmap -layout us,ua -option 'grp:alt_shift_toggle'", false)
        end
    },
    awful.key {
        modifiers = { mod.super },
    ...
    awful.key {
        ...
        on_press = function()
            awful.spawn.easy_async_with_shell("playerctl next", function(stdout)
                awesome.emit_signal("media::next_track")
            end)
        end
    },
    awful.key {
        ...
        on_press = function()
            awful.spawn.easy_async_with_shell("playerctl play-pause", function(stdout)
                awesome.emit_signal("media::play_pause")
            end)
        end
    },
    awful.key {
        ...
        on_press = function()
            awful.spawn.easy_async_with_shell("playerctl previous", function(stdout)
                awesome.emit_signal("media::prev_track")
            end)
        end
    },
    awful.key {
    ...
    awful.key {
        ...
        group = 'launcher',
        on_press = function() awful.spawn("redshift -P -O 3500", false) end
    },
    awful.key {
        modifiers = { mod.super },
        key = 'XF86MonBrightnessUp',
        description = 'turn off night shift',
        group = 'launcher',
        on_press = function() awful.spawn("redshift -P -O 6500", false) end
    },
}

--add to layout left and right
awful.keyboard.append_global_keybindings({
